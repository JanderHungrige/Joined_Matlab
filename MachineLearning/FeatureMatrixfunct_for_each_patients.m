...
function [Matrix]=FeatureMatrixfunct_for_each_patient(Neonate,win,Matrix,saving)
...
%%%%% Adding features of window 180, 300 to features of window 60                   
       ...
       clearvars -except Neonate win Matrix saving counter j L Lf Matrixtemp
    end %win
    ...
    %%%%%%%%%%%%Saving
           if saving
             ...
             if exist('Matrix','var')==1
              save([folder 'SW_feature_Matrix_patient_' num2str(Neonate)],'Matrix');
             end
           end
     k=size(Matrix);k=k(1,1);     
    for k = 54
        Matrix=[];
    end
